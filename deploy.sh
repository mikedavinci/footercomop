if [ "$1" = "dev" ]; then
  export host="dev4.nobleapplications.com"
  ...
  if [ "$1" = "prod" ]; then
    export host="ec2-54-91-66-178.compute-1.amazonaws.com"
    export user="ec2-user"
...
if [ "$1" = "prod" ]; then
  ...
  rsync -Pav -e "ssh -i $pemfile" build/libs/inspirewi.war $user@$host:~/inspirewi.war
fi
...
if [ "$1" = "prod" ]; then
  ...
  ssh -o "ProxyCommand=ssh -i $proxyPem -W %h:%p $proxyUser@$proxyHost" -i $pemfile $user@$host "sudo tail -f /var/log/tomcat9/catalina.`date -u +%Y`-`date -u +%m`-`date -u +%d`.log"
	...
	ssh -i $pemfile $user@$host "sudo rm -rf /usr/share/tomcat9/webapps/inspirewi"
	ssh -i $pemfile $user@$host "sudo cp inspirewi.war /usr/share/tomcat9/webapps/inspirewi.war"
